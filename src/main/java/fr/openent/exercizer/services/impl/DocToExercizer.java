                ...
                if (response.statusCode() == 200) {
                    response.body().onSuccess(body -> {
                        try {
                            ...
                            if (taskId == null) throw new IllegalArgumentException("task_id missing");
                            log.info("Generation task launched with ID: " + taskId);
                                        ...
                                        sseResponse.bodyHandler(sseBody -> {
                                            try {
                                                ...
                                                for (String block : eventBlocks) {
                                                            ...
                                                            if (line.startsWith("data:")) {
                                                                ...
                                                                if (!jsonData.isEmpty()) {
                                                                    try {
                                                                        ...
                                                                        foundJson = true;
                                                                        log.info("JSON retrieved and parsed successfully");
                                                                    } catch (Exception e) {
                                                                        log.error("Failed to parse JSON: " + e.getMessage());
                                                                        this.handleError("JSON parsing error: " + e.getMessage(), 502, handler);
                                                                    }
                                                                    break;
                                                                }
                                                            }
                                                    ...
                                                    if (foundJson) {
                                                            log.info("SSE processing completed successfully, sending result");
                                                        ...
                                                        for(String line : blockLines){
                                                            if(line.startsWith("_")){
                                                                ...
                                                                try {
                                                                    ...
                                                                    this.handleError("SSE response parsing error: " + line, code, handler);
                                                                }
                                                                catch (Exception e){
                                                ...
                                                if(!foundJson) {
                                                    log.warn("No JSON found in SSE response");
                                                    this.handleError("No JSON found in SSE response", 502, handler);
                                                }
                                            } catch (Exception e) {
                                                this.handleError("SSE response parsing error: " + e.getMessage(), 502, handler);
                                            }
                                        });
                                    } else {
                                        this.handleError("SSE GET error: " + sseResponse.statusMessage(),
                                                sseResponse.statusCode(), handler);
                                    }
                                })
                                .onFailure(err -> {
                                    this.handleError("SSE GET error: " + err.getMessage(), 500, handler);
                                });
                        } catch (Exception ex) {
                            this.handleError("launch-generation response parsing error: " + ex.getMessage(), 500, handler);
                        }
                    });
                } else {
                    this.handleError("launch-generation POST error: " + response.statusMessage(),
                            response.statusCode(), handler);
                }
            })
            .onFailure(err -> {
                this.handleError("launch-generation POST error: " + err.getMessage(), 500, handler);
            });
    }
